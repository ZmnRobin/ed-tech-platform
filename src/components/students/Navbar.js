...
import { userLoggedOut } from '../../features/user/auth/authSlice'
...
export default function Navbar() {
    ...
    return (
        <div>
            {user?.role!=='admin' ? <nav class="shadow-md">
                <div class="max-w-7xl px-5 lg:px-0 mx-auto flex justify-between py-3">
                    <img class="h-10" src={portal} alt='Portal' />
                    <div class="flex items-center gap-3">
                        <Link to="/leaderboard" class="font-bold">Leaderboard</Link>
                        <h2>{user?.name}</h2>
